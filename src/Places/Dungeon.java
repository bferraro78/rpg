...
public class Dungeon { 
	...
	private Space start = new Space(0, 0, null);
	private int heroX;
	private int heroY;
	private Space end;
	...
	private final int ITEMGENERATEPERCENT = 10;
	...
	public Dungeon(int dungeonLevel, int herox, int heroy) {
		...
		map[heroX][heroY] = new Space(0, 0, null); // HERO IS A SPACE WITH 0,0
	}
	...
	protected int moveHero(String dir, dude mainCharacter) {
		...
		if (!(newX >= 0 && newX < getSize() && newY >= 0 && newY < getSize()) || (getMap()[newX][newY] == null) ) {
			...
			if (getMap()[newX][newY].getItem() != null) {
				System.out.println("Picked up: " + getMap()[newX][newY].getItem().toString());
				mainCharacter.addToInventory(getMap()[newX][newY].getItem());
			}
			/* Move hero */
			getMap()[newX][newY] = new Space(0, 0, null);
			getMap()[heroX][heroY] = new Space(100, 100, null);
			this.heroX = newX;
			this.heroY = newY;
			code = 1;
		}
		/* Add to steps already taken */
		mainCharacter.addStep(new Space(newX, newY, null));
		return code;
	}
	...
	private Space[][] generateRandomDungeon(int x, int y) {
		...
		/* Set Start **/
		m[getStart().getX()][getStart().getY()] =  new Space(100,100, null);
		...
		while (x != getSize()-1 && y != getSize()-1) {
			...
			while (valid) {
				...
				//TODO Drop Armor/Wep also?
				boolean item = false;
				int itemGenerated = itemGenerator.nextInt(100)+1;
				...
				if (direction >= 0 && direction < 25) { // Create Space UP Direction
					if (x-1 >= 0 && x-1 < getSize() && y >= 0 && y < getSize() && m[x-1][y] == null) { // Check bounds, if not in bounds, try another move
						if (item) {
							m[x-1][y] = new Space(100, 100, ItemDictionary.generateRandomItem());
						} else {
							m[x-1][y] = new Space(100, 100, null);
						}
				...
				if (direction >= 26 && direction < 50) { // Create Space RIGHT Direction
					if (x >= 0 && x < getSize() && y+1 >= 0 && y+1 < getSize() && m[x][y+1] == null) { // Check bounds, if not in bounds, try another move
						if (item) {
							m[x][y+1] = new Space(100, 100, ItemDictionary.generateRandomItem());
						} else {
							m[x][y+1] = new Space(100, 100, null);
						}
						y = y+1;
						valid = false;
					}
	...
	public void printMapStepsTaken(dude mainCharacter) {
		for (int x = 0; x < getSize(); x++) {
			for (int y = 0; y < getSize(); y++) {
				if (mainCharacter.containsSpace(new Space(x, y, null)) || (x == 0 && y == 0)) {
					if (getMap()[x][y] == null) { // No spot, fight monster
						System.out.print(" N ");
					} else if (x == getEnd().getX() && y == getEnd().getY()) { // END
						System.out.print(" E ");
